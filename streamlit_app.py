...
selected_name, selected_more_name, selected_job, selected_race, selected_age = show_character_inputs(job_classes, races,
                                                                                                     age_range)
...
if selected_count == 10:
    if st.button("Save", key="save"):
        save_char(bob, selected_per_row.items(), job_markings, selected_age, selected_race, selected_more_name, avg,
                  selected_job)
    ...
    for column_name, row_label in selected_per_row.items():  # Iteriere durch alle Spalten
        try:
            ...
            if column_name in job_markings and row_label_int < 90:
                st.info(f"**{SHORTS[column_name].value}:** \t{row_label:<25} wird auf 90 angehoben (Haupteigenschaft)",
                        icon="🚨")
